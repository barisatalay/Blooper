...
Not: v1 listesi düz listedir (son 30 grup, en yeni üstte) — gün-gün bölüm başlıkları bilinçli olarak v2'ye bırakıldı; spec bu kararla hizalanmıştır.
...
struct Notifier {
    ...
    // bundleIdentifier DOLU gelir ("com.apple.dt.xctest.tool") — env kontrolü de şart
...
UI metinleri tek dilde (İngilizce) tutulur — dağıtılan ürün İngilizce pratik yapan kullanıcıya hitap eder.

- [ ] **Step 4: Claude Code bulunamadı uyarısı**

`MenuView`'a (translocation uyarısının yanına):
...
Not: bu liste checker.sh'ın çözümleme sırasının UI karşılığıdır; salt bilgilendirme (checker kendi çözümlemesini yapar).

- [ ] **Step 5: Derleme + testler** — Run: `swift build && swift test` → yeşil (xctest guard'ı sayesinde Notifier test sürecinde notification center'a dokunamaz; testler zaten Notifier'ı çağırmaz).

- [ ] **Step 6: Commit**

```bash
git add -A && git commit -m "feat: add notifications, settings toggles and environment warnings"
```
